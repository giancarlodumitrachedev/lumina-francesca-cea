...
export async function getDemoParams(searchParamsPromise: Promise<{ [key: string]: string | string[] | undefined }> | undefined): Promise<DemoParams> {
  ...
  const rawName = "Dott.ssa Francesca Cea";
  const rawCity = "Milano";
  const rawAddress = "Via Luigi Ornato, 72, 20162 Milano MI";
  const rawField = "psicologa di orientamento sistemico-relazionale";
